...
class Condition{
    ...
    public static function getCondition($ids){
        ...
        foreach ($ids as $id) {
            ...
            if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    $tmp_condition = new Condition($row['id'],$row['deviceId'],$row['operator'],$row["value"],$row["notificationId"],$row["message"], $row["sent"], $row["last_sent"]);
    ...
    */
    public static function getConditionsByDevice($deviceId) {
    ...
    */
    ...
    public static function createCondition($deviceId,$operator,$value,$notificationId,$message) {
        ...
        $query = "SELECT id FROM condition ORDER BY id DESC LIMIT 1";
        $stmt = $GLOBALS["dbconnec"]->prepare($query);
        $stmt->execute(array());
        $id = 0;
        if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $tmpInstance = new Condition($id, $deviceId, $operator, $value, $notificationId, $message);
