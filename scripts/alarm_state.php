...
if($type=="myfox"){
    ...
    if((isset($securityState->status) && $securityState->status == "KO" && $securityState->error == "invalid_token") ||  !isset($securityState->status){
        $token=getToken();
        $curl = curl_init( "https://api.myfox.me:443/v2/site/".$siteid."/security?access_token=".$token);
        curl_setopt( $curl, CURLOPT_RETURNTRANSFER, 1);
        $securityState = curl_exec( $curl );
    ...
    if(!isset($securityState->payload->statusLabel)){
        echo "MyFOX Alarm Error: can't have status";
        return false;
    }
